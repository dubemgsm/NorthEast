...
def generate():
    ...
    start = int(data.get('start', 2020))
    end = int(data.get('end', 2024))

    print(f"Generating for {state}, {country}...")

    # Logic from generate_location.py
    lat, lon = 9.0820, 8.6753
    if "nigeria" not in country.lower():
    ...
    for i in range(15):
        ...
        points.append({"lat": p_lat, "lon": p_lon, "color": color, "name": name, "size": size, "desc": "Status: Active"})
    ...
    with open("More/results_map.html", "w") as f:
        f.write(MAP_TEMPLATE.format(state=state, country=country, lat=lat, lon=lon, zoom=7, geojson=json.dumps(mock_geojson), points=json.dumps(points)))

    with open("More/results_charts.html", "w") as f:
        f.write(CHARTS_TEMPLATE.format(state=state, country=country, start=start, end=end, labels=json.dumps(years), data1=json.dumps(data1), data2=json.dumps(data2), sub_labels=json.dumps(sub_labels), sub_data=json.dumps(sub_data)))

    return jsonify({"success": True, "redirect": "results_map.html"})
